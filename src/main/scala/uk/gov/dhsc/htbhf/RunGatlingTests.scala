...
object RunGatlingTests {
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      System.out.println("Finished running gatling tests with feature toggles: " + sys.env.get("FEATURE_TOGGLES"))
      // write the result to system.out so we can identify success or failure from the logs
      // DO NOT EDIT LINE BELOW. This is exact string is searched for in other scripts.
